...
Welcome2.__table__.create(checkfirst=True)


def get_current_welcome_settings2(chat_id):
    try:
        return SESSION.query(Welcome2).filter(Welcome2.chat_id == str(chat_id)).one()
    except:
        return None
    finally:
        SESSION.close()
...
def rm_welcome_setting2(chat_id):
    rem = SESSION.query(Welcome2).get(str(chat_id))
    if rem:
        SESSION.delete(rem)
        SESSION.commit()


def update_previous_welcome2(chat_id, previous_welcome):
    row = SESSION.query(Welcome2).get(chat_id)
    row.previous_welcome = previous_welcome
    # commit the changes to the DB
    SESSION.commit()
