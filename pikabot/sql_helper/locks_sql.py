...
Locks2.__table__.create(checkfirst=True)


def init_locks2(chat_id, reset=False):
    curr_restr = SESSION.query(Locks2).get(str(chat_id))
    if reset:
        SESSION.delete(curr_restr)
        SESSION.flush()
    restr = Locks(str(chat_id))
...
def update_lock2(chat_id, lock_type, locked):
    curr_perm = SESSION.query(Locks2).get(str(chat_id))
    if not curr_perm:
        curr_perm = init_locks(chat_id)
    if lock_type == "bots":
        curr_perm.bots = locked
...
def is_locked2(chat_id, lock_type):
    curr_perm = SESSION.query(Locks2).get(str(chat_id))
    SESSION.close()
    if not curr_perm:
        return False
...
def get_locks2(chat_id):
    try:
        return SESSION.query(Locks2).get(str(chat_id))
    finally:
        SESSION.close()
